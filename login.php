...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $login = trim($_POST['login']); // Can be email or student ID
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT id, student_id, email, password, first_name, last_name FROM users WHERE $field = ?");
        $stmt->execute([$login]);
        $user = $stmt->fetch();
            ...
            <div class="w-full md:w-3/5 p-8 bg-white">
                ...
                <form method="POST" action="login.php" class="space-y-4">
                    <div class="form-group">
                        <label for="login" class="block text-sm font-medium text-gray-700 mb-1">Email or Student ID <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <input type="text" id="login" name="login" required
                                   ...
                                   placeholder="Enter your email or student ID" value="<?php echo isset($_POST['login']) ? htmlspecialchars($_POST['login']) : ''; ?>">
